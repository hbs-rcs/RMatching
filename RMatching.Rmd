...
Let's install (if needed) and load R packages that will aid with data wrangling and 
...
performance-oriented environment would equip hard-to-employ people to get and hold normal jobs. 
...
## Note that this data set is a subset of the original data, excluding subject with missing information
## on prior earnings
...
2. Matching
3. Checking balance post-matching; repeat 2-3 as needed
...
Note that it is only appropriate to control for predictors that are not affected by the treatment 
...
can be misleading and their use is discouraged by leading methodologists. `bal.tab()` also 
identifies types of each variable and applies appropriate techniques for calculating (or not)
standardized difference and other summaries.
...
[Guide to the `cobalt` Package](https://cran.r-project.org/web/packages/cobalt/vignettes/cobalt_A0_basic_use.html). 
...
The new combined data set contains 2,675 subjects, including 185 treated from NSW data. 
...
```{r}
# Treatment effect using regression adjustment
raw_mod <- lm(re78 ~ treat + age + education + black + hispanic + nodegree + married + re74 + re75, 
                data = nsw_PSID)
summary(raw_mod)
confint(raw_mod)
```
...
nonoverlap also make the assumption about a constant treatment effect less plausible
and results in heavy extrapolations. 
...
Examing "methods" options and "distance"
-->
...
```
Now we are ready to rerun the OLS and estimate the causal treatment effect:
...
Note that the estimated treatment effect (\$1,084) is closer to the truth (\$1,676) and its 
95% CI contains the true value.
...
in the background. Read more about other functionalities here:
...
of all covariates in the two groups. 
Perfect (up to discretization) global balance result in L1 = 0. Whereas, L1 = 1 indicates complete separation of 
the multidimensional histograms. Any value in the interval (0, 1) indicates the amount of difference between 
frequencies of units in strata for each coarsened covariate of the two groups.
...
<!-- Again, every package offers slightly different summary of imbalance it takes a bit of time reading thorugh (sparse) documentation to figure it out
...
Note that, unlike NNM in `MatchIt`, CEM method dropped a few (`r m_cem_out$tab["Unmatched","G1"]`) treated units 
(and a vast majority of control units) on strata that did not have any comparables from the other groups. 
...
also allows one to specify the minimal number of breaks of each variable (the default
limit being 1). 
...
It takes CEM output and applies a `model` of user's choice, using a user-specified `formula` like follows:
...
The estimate of the treatment effect ($1,711), though statistically insignificant, is very 
close to the one obtained from the randomized experiment.
...
These workshops are a work in progress, please provide any feedback to: research@hbs.edu.
